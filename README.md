...
## Функциональность
- В проекте можно добавить изображение с названием в свой список карточек, удалить добавленную карточку из своего списка, либо открыть его в полном разрешении. Пользователь не может удалить изображения другого пользователя.
- Для входа в свой аккаунт для начала необходимо зарегистрироваться, а после регистрации авторизоваться. После чего автоматически переведётся на страницу списка карточек.
Есть возможность сменить имя, деятельность и аватарку своего профиля.
...
* `/blocks` – папка со стилями для каждого элемента
